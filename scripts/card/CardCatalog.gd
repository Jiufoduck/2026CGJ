...
# - REWARD_POOL_IDS_BY_PLAYER：每名玩家奖励池 ID。奖励池包含该玩家全部卡牌和恢复牌。
...
const INITIAL_DECK_IDS_BY_PLAYER := {
	...
	2: ["b7_knockback", "b12_damage_boost", RESTORE_CARD_ID],
}
...
static func make_reward_choices(player_id: int, rng: RandomNumberGenerator = null) -> Array:
	var reward_ids: Array = REWARD_POOL_IDS_BY_PLAYER.get(player_id, []).duplicate()
